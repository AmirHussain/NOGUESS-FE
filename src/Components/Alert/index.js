                            ...
                            {ala && !ala.hidden && (
                                <div className={classes.alertStyle}>
                                    ...
                                    <Collapse in={!ala.hidden}>
                                        <Alert onClose={() => removeAlert(index)}

                                            severity={ala.severity}>
                                            <AlertTitle sx={{ display: 'flex' }}> {ala.severity === 'info' && (
                                                <CircularProgress sx={{ width: '24px !important', height: '24px  !important' }} />
                                            )}
                                                <span style={{ marginLeft: '5px' }}>{ala.title}</span>
                                            </AlertTitle>
                                            {ala.description} {ala.url && (<>— <strong><a href={ala.url} target="_blank" rel="noreferrer">view on blockexplorer!</a></strong></>)}
                                        </Alert>
                                    </Collapse>
                                </div>
                            )}
