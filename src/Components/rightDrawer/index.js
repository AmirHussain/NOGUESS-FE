...
import ClaimItem from '../../pages/staking/claimReward';
const style = {
  position: 'absolute',
  top: '50%',
  left: '50%',
    ...
    <React.Fragment key="RIGHT1">
      ...
      >
        <Fade in={params.drawerOpen}>
          ...
          >
            <AppBar key="rightbar" position="relative" className={classes.rightDrawerHeader} sx={{ height: { xs: 'auto', md: '77px' } }} color="primary">
              <Toolbar
                variant="dense"
                sx={{
                  height: '100%',
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
                  <Avatar key="rightDrawerAvatar" aria-label="Recipe" className={classes.avatar} sx={{ marginRight: '10px' }}>
                    <img className={classes.avatar} alt="" src={currentRow?.token?.icon} />
                  </Avatar>
                  <Typography variant="h5">{params.Opration}</Typography>
                </div>
              </Toolbar>
            ...
            {params.component === 'staking' && (params.Opration === 'Staking' ? <StakeItem input={params}></StakeItem> : <ClaimItem input={params}></ClaimItem>)}

            {params.component === 'SupplyItem' && <SupplyItem input={params}></SupplyItem>}

            {params.component === 'borrowItem' && <BorrowItem input={params}></BorrowItem>}
            {/* </Drawer> */}
          </Box>
        </Fade>
      </Modal>
