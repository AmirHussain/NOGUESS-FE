...
import { bigToDecimal, bigToDecimalUnits, decimalToBigUnits } from '../../utils/utils';
...
const options = {
  ...
  tooltip: {
    ...
    formatter: function () {
      if (this.series && this.series.chart && this.series.chart.options && this.series.chart.options.series && this.series.chart.options.series.length) {
        return `
...
export default function Asset(params) {
  ...
  const setApyGraph = async (lendingContract) => {
    ...
    try {
      console.log(currentToken.address, tokenIntrestRateModal, decimalToBigUnits((Number(bigToDecimalUnits(tokenBorrowLimitations.MAX_UTILIZATION_RATE, 2)) + 0.01).toString(), 2));
      console.log(lendingContract);
      const chartData = await lendingContract.getChartData(
        currentToken.address,
        IntrestRateModal,

        decimalToBigUnits((Number(bigToDecimalUnits(tokenBorrowLimitations.MAX_UTILIZATION_RATE, 2)) + 0.01).toString(), 2),
      );
  ...
  const getTokenMarketDetails = async (tokendetails) => {
    ...
    const supplyAPR = await lendingContract.calculateCurrentLendingProfitRate(tokendetails?.token.address, tokenIntrestRateModal);
    ...
    const borrowAPR = await lendingContract.getOverallBorrowRate(tokendetails?.token.address, borrowRatesResult[0], borrowRatesResult[1]);
    details['uratio'] = bigToDecimalUnits(uratio, 2) * 100;
    currentUtilization = details['uratio'];
    details['supplyAPR'] = bigToDecimalUnits(supplyAPR, 2) * 100;
    details['borrowAPR'] = bigToDecimalUnits(borrowAPR, 2) * 100;
    currentSupplyAPR = details['supplyAPR'];
    currentBorrowAPR = details['borrowAPR'];
    details['supplyAPY'] = getAPY(bigToDecimalUnits(supplyAPR, 2)) * 100;
    details['borrowAPY'] = getAPY(bigToDecimalUnits(borrowAPR, 2)) * 100;
          ...
          <Grid item md={8} xs={8}>
            <Grid item md={12} xs={12}>
              <Card className={classes.innerCard}>
                ...
                {callInProgress && <h4>Please wait intrest rate model is calculating</h4>}
              </Card>
            </Grid>
            <Grid item md={12} xs={12} sx={{ marginTop: '16px' }}>
              <Card className={classes.innerCard}>
                <div className="text">
                  <div className={classes.textHighlighted}>Stats</div>
            ...
            <Grid item md={12} xs={12} sx={{ marginTop: '16px' }}>
              <Card className={classes.innerCard}>
                ...
                <div className="d-flexSpaceBetween">
                  ...
                  <div>
                    <b>{tokendetails?.borrowLimitation?.MAX_UTILIZATION_RATE}%</b>
                  </div>
                </div>
                ...
                <div className="d-flexSpaceBetween">
                  ...
                  <div>
                    <b>{tokendetails?.borrowLimitation?.OPTIMAL_UTILIZATION_RATE}%</b>
                  </div>
                </div>
                ...
                <div className="d-flexSpaceBetween">
                  ...
                  <div>
                    <b>{tokendetails?.borrowLimitation?.StableRateSlope1}%</b>
                  </div>
                </div>
                ...
                <div className="d-flexSpaceBetween">
                  ...
                  <div>
                    <b>{tokendetails?.borrowLimitation?.VariableRateSlope1}%</b>
                  </div>
                </div>
                <div className="d-flexSpaceBetween">
                  <div className={classes.textMuted}>VariableRateSlope2</div>
                  <div>
                    <b>{tokendetails?.borrowLimitation?.VariableRateSlope2}%</b>
                  </div>
                </div>
                ...
                <div className="d-flexSpaceBetween">
                  ...
                  <div>
                    <b>{tokendetails?.borrowLimitation?.baseRate}%</b>
                  </div>
                </div>
                ...
                <div className="d-flexSpaceBetween">
                  ...
                  <div>
                    <b>{tokendetails?.borrowLimitation?.AllowStableJob?.toString()}</b>
                  </div>
                </div>
              </Card>
            </Grid>
            <Grid item md={12} xs={12} sx={{ marginTop: '16px' }}>
              <Card className={classes.innerCard}>
                <div className="text">
                  <div className={classes.textHighlighted}>Adaptive Limits</div>
                  <div className={classes.textMuted}>No Limits</div>
                </div>
              </Card>
            </Grid>
          </Grid>
          <Grid item md={4} xs={4}>
            <Card className={classes.innerCard}>
              ...
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>
                    {tokenSummary?.lendings} {currentToken?.symbol}
                  </b>
                </div>
              </div>
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>
                    {tokenSummary?.totalDebt} {currentToken?.symbol}
                  </b>
                </div>
              </div>
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>
                    {tokenSummary?.totalVariableDebt} {currentToken?.symbol}
                  </b>
                </div>
              </div>
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>
                    {tokenSummary?.totalStableDebt} {currentToken?.symbol}
                  </b>
                </div>
              </div>
              ...
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>{parseFloat(tokenSummary?.uratio || 0).toFixed(3)}%</b>
                </div>
              </div>
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>
                    {parseFloat(tokenSummary?.supplyAPR || 0).toFixed(3)}% / {parseFloat(tokenSummary?.supplyAPY || 0).toFixed(3)}%
                  </b>
                </div>
              </div>
              <div className="d-flexSpaceBetween">
              ...
              <div className="d-flexSpaceBetween">
                <div className={classes.textMuted}>Rewards APR</div>
                <div>
                  <b>0.06%</b>
                </div>
              </div>
              ...
              <div className="d-flexSpaceBetween">
                ...
                <div>
                  <b>3P8G747fnB1DTQ4d5uD114vjAaeezCW4FaM</b>
                </div>
              </div>
              <div className="d-flexSpaceBetween">
                <div className={classes.textMuted}>Borrow</div>
                <div>
                  <b>{currentToken?.symbol}</b>
                </div>
              </div>
            </Card>
          </Grid>
