...
export default function AddUpdateAdaptiveLimitation(params) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await governanceContract.RemoveTokenAdaptiveLimitations(updatedRow.tokenAddress, params.rowIndex, { gasLimit: 100000 });
  ...
  return (
    <div>
      <Dialog open={params.open} onClose={params.handleClose} sx={{ zIndex: 1100, maxWidth: '100vw', minWidth: '60vw' }}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center' }}>{params.newRow ? 'Add' : 'Update'} Adaptive Limitation</DialogTitle>
        <DialogContent>
          ...
          <Grid container direction="row" spacing={2}>
            <Grid item xs={6} sm={6} md={6}>
              <FormControlLabel
                control={<Checkbox checked={updatedRow?.Replenish} onChange={(e) => handleValueChange(e.target.value === 'on', 'Replenish')} name="Replenish" />}
                label="Replenish"
              />
            </Grid>
            <Grid item xs={6} sm={6} md={6}>
              <FormControlLabel
                control={<Checkbox checked={updatedRow?.Redeem} onChange={(e) => handleValueChange(e.target.value === 'on', 'Redeem')} name="Redeem" />}
                label="Redeem"
              />
            </Grid>
          </Grid>
        ...
        <DialogActions>
          {!params.newRow && (
            <Button sx={{ position: 'absolute', left: '10px' }} onClick={() => handleDelete()} variant="contained" color="error">
              Delete
            </Button>
          )}
          ...
          <Button onClick={() => saveAggregator()} variant="contained">
            Save
          </Button>
        </DialogActions>
      </Dialog>
