...
export default function Marketitem(props) {
  ...
  React.useEffect(() => {}, [tokenSummary]); // Empty array means to only run once on mount.
  ...
  const setSupplyAPR = async (tokendetails, lendingContract) => {
    if (tokendetails.aggregator) {
      const supplyAPR = await lendingContract.calculateCurrentLendingProfitRate(tokendetails.token.address, IntrestRateModal);
        ...
        <CardHeader
          ...
          avatar={
            <Avatar sx={{ cursor: 'pointer' }} aria-label="Recipe" className={classes.avatar}>
              <img className={classes.avatar} alt="" src={row?.icon} />
            </Avatar>
          }
        ...
        <CardContent className={classes.cardContent} sx={{ paddingLeft: '0px', paddingRight: '0px' }}>
          <Grid container direction="row" justifyContent="start" alignItems="flex-left" spacing={1} style={{ width: '100%', textAlign: 'left' }}>
            <Grid item xs={3} sm={3} md={3}>
              ...
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>$ {tokenSummary?.lendingsInUSD ? parseFloat(tokenSummary?.lendingsInUSD || 0).toFixed(5) : 0}</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>$0.12M FToken</div>
            </Grid>
            <Grid item xs={3} sm={3} md={3}>
              <div className={classes.textMuted}>Suppy APY</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>{tokenSummary?.supplyAPY ? parseFloat(tokenSummary?.supplyAPY || 0).toFixed(5) : 0}%</div>
            </Grid>

            <Grid item xs={3} sm={3} md={3}>
              <div className={classes.textMuted}>Liquidity</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>$123M</div>
            </Grid>
            <Grid item xs={3} sm={3} md={3}>
              <div className={classes.textMuted}>Total Borrow</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>$ {tokenSummary?.totalDebtInUSD ? parseFloat(tokenSummary?.totalDebtInUSD || 0).toFixed(5) : 0}</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>$ {tokenSummary?.totalDebtInUSD ? parseFloat(tokenSummary?.totalDebtInUSD || 0).toFixed(5) : 0}</div>
            </Grid>
            <Grid item xs={3} sm={3} md={3}>
              <div className={classes.textMuted}>Borrow APY</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>{tokenSummary?.borrowAPY ? parseFloat(tokenSummary?.borrowAPY || 0).toFixed(5) : 0}%</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>{tokenSummary?.borrowAPY ? parseFloat(tokenSummary?.borrowAPY || 0).toFixed(5) : 0}%</div>
            </Grid>
            <Grid item xs={3} sm={3} md={3}>
              <div className={classes.textMuted}>collateral Factor</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>{tokendetails?.borrowLimitation?.CollateralFator}%</div>
            </Grid>
            <Grid item xs={3} sm={3} md={3}>
              <div className={classes.textMuted}>Price</div>
              <div sx={{ color: theme.lightText, fontWeight: 600 }}>$ {tokenSummary?.priceInUSD ? parseFloat(tokenSummary?.priceInUSD || 0).toFixed(5) : 0}</div>
            </Grid>
          </Grid>
        </CardContent>
      </Card>
