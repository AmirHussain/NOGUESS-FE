...
function Governance() {
  ...
  return (
    <>
      ...
      <Grid container direction="row" justifyContent="center" alignItems="flex-center" spacing={2} style={{ width: '100%', textAlign: 'left' }}>
        <Grid item xs={8} sm={8} md={8}>
          <div className="d-flexSpaceBetween">
            ...
            <Button
              sx={{ fontSize: 14, fontWeight: 600, color: theme.lightText, paddingBottom: '10px', cursor: 'pointer' }}
              onClick={() => handleClickOpen()}
              disabled={supplyAmount < votingWeightage}
            >
              + create proposal
            </Button>
          </div>
                  ...
                  <Grid container direction="row" justifyContent="start" alignItems="flex-left" spacing={1} style={{ width: '100%', textAlign: 'left', margin: 0 }}>
                    <Grid item xs={9} sm={9} md={9} style={{ borderRight: '0.5px solid ' + theme.borderColor, padding: 20 }}>
                      <Typography sx={{ fontSize: 12, fontWeight: 500, paddingBottom: '28px' }} variant="h4">
                        <span className={classes.chip}># {r.id}</span>
                        <Typography sx={{ fontSize: 11, width: '100%', color: theme.lightText, textAlign: 'left' }} variant="p">
                          {!r.userVoted ? 'Not voted' : ''}
                        </Typography>
                        {r.status === ProposalStatus.active && <Chip sx={{ float: 'right' }} label={r.status} color="success" variant="outlined" />}
                      </Typography>
                    ...
                    <Grid item xs={3} sm={3} md={3} sx={{ margin: 'auto', textAlign: 'center' }}>
                      <div className="d-flexSpaceAround">{renderStatus(r)}</div>
          ...
          <Card className={classes.card}>
            <CardContent className={classes.cardContent} sx={{ paddingTop: '0px !important', paddingBottom: '0px !important', textAlign: 'left' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', paddingBottom: '20px' }}>
                <span style={{ paddingLeft: '4px', fontSize: '20px', fontWeight: '600' }}>{votingWeightage || 0.0}</span>
              </div>
              <Typography sx={{ fontSize: 14, fontWeight: 600, paddingBottom: '40px', color: theme.lightText }} variant="p">
                Total Locked
              </Typography>
              <div style={{ display: 'flex', alignItems: 'center', paddingBottom: '20px' }}>
                <Avatar sx={{ cursor: 'pointer' }} aria-label="Recipe" className={classes.avatar}>
                  <img className={classes.avatar} alt="" src={Icons.ethereum} />
                </Avatar>{' '}
                <span style={{ paddingLeft: '4px', fontSize: '20px', fontWeight: '600' }}>{supplyAmount || 0.0}</span>
              </div>
            ...
            <CardContent className={classes.cardContent} sx={{ paddingTop: '0px !important', paddingBottom: '0px !important', textAlign: 'left' }}>
              ...
              <Typography sx={{}} variant="p">
                <ol style={{ paddingLeft: 12 }}>
                  <li> Deposit your tokens in the XVS Vault</li>
                  <li>Delegate your voting power to yourself or somebody else</li>
                </ol>
              </Typography>
            </CardContent>
          </Card>
