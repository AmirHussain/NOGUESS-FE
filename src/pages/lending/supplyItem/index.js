...
export default function SupplyItem(params) {
  ...
  const redeemAmount = async (row) => {
    ...
    try {
      ...
      const result = await lendingContract.redeem(currentRow.token.symbol, decimalToBig(row.tokenAmount), currentRow.token.address, row.id, IntrestRateModal, {
        gasLimit: 1000000,
      });
  ...
  const startSupply = async () => {
    ...
    try {
      ...
      const wethResult = await weth.approve(lendingContract.address, decimalToBig(amount), { gasLimit: 1000000 });
      ...
      console.log(decimalToBig(amount), decimalToBigUnits(lockDuration.toString(), 0));
      const result = await lendingContract.lend(supplyToken, pedgeToken, decimalToBig(amount), decimalToBigUnits(lockDuration.toString(), 0), { gasLimit: 1000000 });
      ...
      >
        ...
        >
          <div className="d-flexSpaceBetween">
            {' '}
            <span>SUPPLY APY:</span> <span>{currentRow.rate}</span>
          </div>
          <div className="d-flexSpaceBetween">
            {' '}
            <span>WALLET:</span> <span>{currentRow.supplyAmount}</span>
          </div>
        </Card>
        <Card
          className={classes.innerCard}
        ...
        >
          <TabContext variant="fullWidth" value={value}>
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
              <TabList onChange={handleChange} indicatorColor="main" textColor="inherit" fontWeight="inherit" aria-label="lab API tabs example" centered>
            ...
            <TabPanel value="1">
                    ...
                    <FormControl variant="standard" sx={{ color: theme.lightText, width: '100%' }}>
                      ...
                      <Input
                        ...
                        startAdornment={
                          ...
                          >
                            <img className="chainIcon" alt="" src={currentRow?.token?.icon} />
                          </Avatar>
                        }
                      />
                    </FormControl>
              ...
              <div variant="dense" className="d-flexCenter" sx={{ height: theme.headerHeight }}>
                <Button sx={{ width: '100%', borderRadius: theme.cardBorderRadius, minHeight: '45px', fontWeight: '600' }} variant="contained" onClick={startSupply}>
                  Supply
                </Button>
              </div>
            </TabPanel>
                      ...
                      <TableRow key={row.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        ...
                        <TableCell align="right">
                          {!row.redeem && row.differnce <= 0 && (
                            <Button variant="contained" size="small" className={classes.actionButton} onClick={() => redeemAmount(row)}>
                              Redeem
                            </Button>
                          )}
                          {!row.redeem && row.differnce > 0 && <h4>After {row.differnce} days </h4>}
                          {row.redeem && <DoneAll></DoneAll>}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
