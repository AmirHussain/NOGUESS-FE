...
function Lending() {
  ...
  const getAllMarketDetails = async () => {
    if (Tokens && Tokens.length && TokenAggregators.length) {
      ...
      const aggregators = TokenAggregators.map((aggtoken) => {
        ...
        if (currentToken) {
          return {
            aggregator: aggtoken.aggregator,
            tokenAddress: currentToken.address,
            decimal: decimalToBigUnits(aggtoken.decimals.toString(), aggtoken.decimals > 9 ? 0 : 0),
          };
        }
      });
      const data = await lendingContract.getCurrentLiquidity(aggregators);
      setCurrentMarket({
  ...
  return (
    <Grid container direction="row" justifyContent="center" alignItems="flex-center" spacing={2}>
        ...
        <Card className={classes.card}>
          ...
          <CardContent className={classes.cardContent} sx={{ paddingTop: '0px !important', paddingBottom: '0px !important', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <p style={{ fontSize: '33px', color: theme.greenColor, fontWeight: 600, textAlign: 'left', margin: '0px', lineHeight: 1 }}>{currentRow?.balanceOf || 0.0} %</p>
            </div>
            ...
            <Grid container direction="row" justifyContent="start" alignItems="flex-left" spacing={1} style={{ width: '100%', textAlign: 'left', paddingTop: '40px' }}>
              <Grid item xs={4} sm={4} md={4} style={{ borderRight: '0.5px solid ' + theme.borderColor }}>
                ...
                <div>
                  <span style={{ paddingLeft: '4px', fontSize: '20px', fontWeight: '600' }}>${currentRow?.apy || 0.0}</span>
                </div>
              </Grid>
              ...
              <Grid item xs={4} sm={4} md={4} style={{ borderRight: '0.5px solid ' + theme.borderColor }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ paddingLeft: '4px', fontSize: '20px', fontWeight: '600' }}>${currentMarket?.totalSupply || 0.0}</span>
                </div>
              </Grid>
              ...
              <Grid item xs={4} sm={4} md={4}>
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <span style={{ paddingLeft: '4px', fontSize: '20px', fontWeight: '600' }}>${currentMarket?.totalDebt || 0.0}</span>
                </div>
              </Grid>
            </Grid>
          </CardContent>
      ...
      {drawerOpen && (
        <RightDrawer
          Opration={currentMethod === 'SupplyItem' ? 'Supply' : 'Borrow'}
          component={currentMethod}
