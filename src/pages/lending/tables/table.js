...
export default function LendingTable(props) {
  ...
  React.useEffect(() => {}, [props?.SupplyRows]);
        ...
        <TableBody>
          {props?.SupplyRows?.map((row) => (
            ...
            >
              ...
              >
                <img className="chainIcon" alt="" src={row.token?.icon} /> <h4>{row.token?.name} </h4>{' '}
              </TableCell>

              <TableCell className={classes.tableCell} align="right">
                <h4> {parseFloat(getAPY((props.market === 'supply' ? row.supplyAPY : row.borrowAPY) || 0) * 100).toFixed(3)} %</h4>
              </TableCell>
              <TableCell className={classes.tableCell} align="right">
                <h5>
                  {(props.market === 'supply' ? row.supplyAmount : row.borrowAmount) || '0.00'} {row.token?.symbol}
                </h5>
              </TableCell>
