  ...
  const startBorrow = async () => {
    ...
    try {
      ...
      const result = await lendingContract.borrow(
        ...
        decimalToBig(amount.toString()),
        decimalToBig(colleteralAmount.toString()),
        decimalToBig(currentRow.borrowAPY.toString() || '0'),
        stableRate,
        { gasLimit: 1000000 },
      );
  ...
  const repayAmount = async (row) => {
    ...
    try {
      ...
      const result = await lendingContract.repay(
        row['loanToken'],
        decimalToBig(repayAmountValue.toString()),
        currentToken.address,
        currentCollateralToken.address,
        row.id,
        IntrestRateModal,
        { gasLimit: 1000000 },
      );
    ...
    try {
      if (amount && collateral) {
        ...
        if (collateralAggregators && loanAggregators) {
          ...
          const totalCollateralInUSD = (Number(totalLoanInUSD) * Number(100)) / (Number(bigToDecimalUnits(TokenBorrowLimitations.CollateralFator, 2)) * 100);
    ...
    <React.Fragment key="RIGHTContent">
      ...
      >
        ...
        >
          <div className="d-flexSpaceBetween">
            {' '}
            <span>Borrow APY:</span> <span>{parseFloat(getAPY(currentRow?.borrowAPY) * 100).toFixed(3)} %</span>
          </div>
        </Card>
        ...
        >
          <TabContext value={value}>
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
              <TabList onChange={handleChange} indicatorColor="main" textColor="inherit" aria-label="lab API tabs example" centered>
            ...
            <TabPanel value="1">
                    ...
                    <FormControl variant="outlined" style={{ m: 1, width: '100%', color: theme.lightText + ' !important' }}>
                      ...
                      <OutlinedInput
                        ...
                        startAdornment={
                          ...
                          >
                            <img className="chainIcon" alt="" src={currentToken.icon} />
                          </Avatar>
                        }
                      />
                    </FormControl>
                  ...
                  <Grid item xs={12} sm={12} md={12}>
                    <Typography sx={{ fontSize: 12, color: theme.lightText }} variant="h4" gutterBottom>
                      <FormControl sx={{ width: '100%', color: theme.lightText + ' !important' }}>
                        <InputLabel sx={{ color: theme.lightText + ' !important' }} id="demo-multiple-checkbox-label">
                          Select Collateral
                        </InputLabel>
                        <Select
                          ...
                          startAdornment={
                            ...
                            >
                              <img className="chainIcon" alt="" src={collateralToken?.icon} />
                            </Avatar>
                          }
                        >
                          {Tokens?.map(
                            (token) =>
                              ...
                              token.address !== currentToken.address && (
                                <MenuItem key={token.name} value={token.address}>
                                  <img className="chainIcon" alt="" src={token.icon} />
                    ...
                    <Typography variant="p" sx={{ display: 'flex', justifyContent: 'space-between' }} gutterBottom>
                      ...
                      <div>
                        {parseFloat(colleteralAmount || 0).toFixed(5)} {collateralToken?.symbol}
                      </div>
                    </Typography>
                    <Typography variant="p" sx={{ display: 'flex', justifyContent: 'space-between' }} gutterBottom>
                      <div> Available amount </div>
                      <div>
                        {parseFloat(availableAmount || 0).toFixed(5)} {collateralToken?.symbol}
                      </div>
                    </Typography>
                  </Grid>
                </Grid>
              ...
              <div variant="dense" className="d-flexCenter" sx={{ height: theme.headerHeight }}>
                <Button sx={{ width: '100%', borderRadius: theme.cardBorderRadius, minHeight: '45px', fontWeight: '600' }} variant="contained" onClick={startBorrow}>
                  Borrow
                </Button>
              </div>
            </TabPanel>
                    ...
                    {borrowDetails?.map((row) => (
                      <TableRow key={row.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        <TableCell component="th" scope="row">
                          <h4>
                            {row.borrowAmount} {currentRow.symbol}
                          </h4>
                        </TableCell>
                        <TableCell component="th" scope="row">
                          <h4>
                            {row.loanAmount} {currentRow.symbol}
                          </h4>
                        </TableCell>
                        ...
                        <TableCell align="right">
                          {row.loanAmount > 0 && (
                            <div>
                              {' '}
                              <Button variant="contained" size="small" className={classes.actionButton} onClick={() => handleOpen(row)}>
                                Repay
                              </Button>
                            </div>
                          )}
                          {row.loanAmount <= 0 && <div>Repaid</div>}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
      ...
      <Modal open={open} onClose={handleClose} aria-labelledby="child-modal-title" aria-describedby="child-modal-description">
        <Box sx={{ ...style, width: 'auto' }}>
          ...
          <Typography sx={{ mt: 2 }}>Amount Borrowed: {redeemRow.borrowAmount}</Typography>
          <Typography sx={{ mt: 2 }}>Remaining: {redeemRow.loanAmount}</Typography>
          <Typography sx={{ mt: 2 }}>Amount To Pay: {redeemRow.loanAmount + redeemRow.loanAmount * currentRow.borrowAPY}</Typography>
          <Typography sx={{ mt: 2 }}>
            <FormControl variant="outlined" sx={{ m: 1, width: 300 }}>
              <InputLabel htmlFor="RepayID">Repay Amount</InputLabel>
              <OutlinedInput
                label="Repay Amount"
